...
export const GuideInfo = ({ guidePage }: Props) => {
	return (
		<div className='container mx-auto px-4 py-8'>
			<h1 className='text-2xl md:text-5xl mb-8'>{guidePage.attributes.title}</h1>
			{guidePage.attributes.body.map((section) => (
				<section key={section.id}>
					{section.images.data?.map((el) => (
						<ImageWithDomain key={el.id} src={el.attributes.url} width={el.attributes.width} height={el.attributes.height} alt={el.attributes.name} />
						))}
